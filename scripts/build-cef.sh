...
sync_sources() {
    ...
    local gclient_marker="$BUILD_DIR/.gclient-$chromium_version"
    ...
    local hooks_marker="$BUILD_DIR/.gclient-hooks-$chromium_version"
    ...
    PATH="$depot_tools:$PATH" python3 \
        "$chromium_source/tools/update_pgo_profiles.py" \
        --target mac-arm \
        update \
        --gs-url-base=chromium-optimization-profiles/pgo_profiles
    PATH="$depot_tools:$PATH" python3 \
        "$chromium_source/v8/tools/builtins-pgo/download_profiles.py" \
        download \
