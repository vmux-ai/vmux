...
- Unit tests live beside their source, not inside it: `foo.rs` declares `#[cfg(test)] #[path = "foo.test.rs"] mod tests;` and the tests go in `foo.test.rs`. `#[path]` on a module declared in a file resolves relative to that file's own directory, so the two are siblings, and nesting is unchanged — `use super::*` still reaches the parent. A file with several test modules gets one file each, named `foo.<topic>.test.rs`; they cannot share a file, because folding them under one wrapper module would repoint every `use super::*` inside them. The exception is `src/bin/*.rs`, where cargo would pick a sibling `.rs` up as another binary target.
