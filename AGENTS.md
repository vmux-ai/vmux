...
- Do not add `macro_rules!` or function-like procedural macros to vmux-owned code. Use ordinary functions, typed systems, and explicit control flow. Attribute and derive macros may express declarative metadata or framework integration. Vendored patches and tuple-arity generation required by Rust remain exceptions.
...
- **A crate lives in `crates/app/`, `crates/page/`, `crates/host/`, or flat in `crates/`.** `app/` is something a user starts — the desktop app, the iOS app, the CLI. `page/` is a crate that answers a URL (`vmux://terminal`, `file://`), one per page; if it does not serve a URL it does not go there. `host/` is runtime that runs with no UI and owns state — the service, the relay client, the MCP crates. Everything else stays flat: shared libraries, `vmux_browser`, which composes pages into the workspace shell, and `vmux_app`, which exposes that composition as the platform-neutral framework facade. A `page/` crate must never depend on `vmux_browser` or `vmux_app`; application crates consume `vmux_app` and add platform adapters. Two traps. `host/` is **not** a layer above `page/`: `page/vmux_agent` depends on `host/vmux_service`, `host/vmux_mcp` and `host/vmux_transport`, because those crates cfg-split and a page links only the non-host half. And the cfg alias `host` is **not** the directory: `vmux_ui` and `page/vmux_layout` hold host-gated code while staying out of `host/`, and `vmux_git` and `vmux_profile` stay flat because page crates and `vmux_core` depend on them. The directory means "this crate's job is host-side work", not "this crate contains host-gated code".
...
- Do not define a function inside another function. Nested functions hide reusable logic inside control flow and make systems read like containers. Put derived behaviour on its owning type; keep framework entry points such as Bevy systems at module scope.
...
- Plugin boundaries represent independently configurable, reusable, platform-gated, or lifecycle-owned features, not every small group of systems. As a rule of thumb, fewer than four closely coupled systems stay in the owning plugin. Prefer ordinary entity/component state, targeted observers, and owner-type methods over a generic micro-plugin or another `App` extension. A smaller plugin is justified only when it has an independent composition or scheduling boundary.
