...
- Declare a file's entry point at the top, above what it composes. For a Bevy module that is its `Plugin`; for a Dioxus page it is the root component (`Page`, `App`). The entry point is the table of contents — what the file spawns, schedules or renders — and reading it first is what makes the rest navigable. Burying it behind helpers hides that.
