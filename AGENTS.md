...
- No comments in the `Makefile`, in `.github/workflows/`, or in a `Cargo.toml` / `Dioxus.toml`. A recipe explaining itself over twelve lines and running in six is the usual shape. If a flag needs a reason, the reason goes in `docs/architecture.md` or nowhere.
- No comments in Rust sources. Not `//`, not `///`, not `//!`. Names and types carry what the code does; `docs/architecture.md` carries why the system is shaped the way it is. A *why* worth writing down goes there, where a reader finds it without opening a file and where one paragraph serves the ten call sites that would each have grown their own. This does not apply to `patches/`, which is vendored third-party code we re-apply on every version bump, nor to `examples/`, whose whole job is to teach — an example with its explanation in another file is one nobody reads. Two lines at the top saying what it shows, and nothing else: no README beside it, no paragraph. If it needs more, the API is wrong.
...
- **A component reads state through one hook, and derives nothing.** A body that opens with a dozen `let`s — matching on the route, counting, folding a trail into a string, building class lists — has put the screen's logic where it cannot be named, reused or tested, and where it re-runs on every render. Take the whole view from a single call, `let view = use_thing();`, and hang the derivation off the type that call returns, so `Thing::of`, `Thing::elide` and friends are ordinary functions a test can reach. One hook, not a handful: `use_a()` plus `use_b()` plus `use_c()` is the same pile with more ceremony, and it makes the order they run in load-bearing. State the framework owns — a route, a signal, a context — still comes from its own hook inside that one; what must not happen is the component doing the arithmetic. If the derivation needs nothing from the framework, it belongs in an ECS system writing a resource instead, and the hook just reads it.
