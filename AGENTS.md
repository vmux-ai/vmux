...
- Declare a module's `Plugin` at the top of its file, above the types and systems it registers. The plugin is the table of contents for the file — what it spawns, initialises and schedules — and reading it first is what makes the rest navigable. Burying it after several hundred lines of helpers hides that.
