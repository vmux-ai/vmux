...
- **A crate lives in `crates/app/`, `crates/page/`, or flat in `crates/`.** `app/` is something a user starts — the desktop app, the iOS app, the CLI. `page/` is a crate that answers a URL (`vmux://terminal`, `file://`), one per page; membership is checkable, so if it does not serve a URL it does not go there. Everything else stays flat: shared libraries, host-side runtime, and `vmux_browser`, which composes pages into the desktop shell and therefore sits above `page/` and below `app/` — a `page/` crate must never depend on it, and nothing but `app/vmux_desktop` may. Note the cfg alias `host` and any future `host/` directory are not the same set: `vmux_ui` and `page/vmux_layout` both contain host-gated code.
