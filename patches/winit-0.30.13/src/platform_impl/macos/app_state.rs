...
impl ApplicationDelegate {
    ...
    fn will_terminate(&self, _notification: &NSNotification) {
        ...
        let mtm = MainThreadMarker::from(self);
        let app = NSApplication::sharedApplication(mtm);
        // Defer teardown so we never call `handle_event` (via `notify_windows_of_exit` or
        // `internal_exit`) while AppKit invoked this callback during an active winit handler
        // borrow — that re-entrancy panics (see `EventHandler::handle_event`).
