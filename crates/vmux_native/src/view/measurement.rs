...
//!
//! The instructions in [`SurfaceElement`](crate::view::surface_element::SurfaceElement) need no reply and
//! resolve the moment they are queued. These need one, and it comes back over `window.ipc` rather
//! than on a request. Unlike the caret in [`event_selection`](crate::view::event_selection), a
