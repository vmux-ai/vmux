...
pub fn PromptComposer(
    ...
    let footer = footer.or_else(|| {
        Some(rsx! {
            div { class: "truncate text-[10px] text-muted-foreground/55",
    ...
    rsx! {
        vmux_chat_ui::PromptComposer {
            ...
            accent_color,
            accent_gradient,
            attach_title: translate("composer-attach-files"),
            remove_attachment_title: translate("composer-remove-attachment"),
            action,
            action_title,
            action_enabled,
            on_input,
            ...
            on_remove_attachment: move |index| {
                on_remove_attachment.call(index);
