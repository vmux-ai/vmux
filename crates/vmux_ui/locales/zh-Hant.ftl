...
schema-update-channel = 發佈頻道
schema-update-channel-detail = 穩定版接收正式發佈，預覽版接收每日建置。
schema-update-channel-stable = 穩定版
schema-update-channel-preview = 預覽版
...
command-rotate-forward = 佈局 > 窗格 > 向前輪換窗格
command-rotate-backward = 佈局 > 窗格 > 向後輪換窗格
command-mirror-panes = 佈局 > 窗格 > 鏡像窗格
command-mirror-panes-horizontal = 佈局 > 窗格 > 水平鏡像窗格
command-mirror-panes-vertical = 佈局 > 窗格 > 垂直鏡像窗格
...
git-changes = 變更
git-branches = 分支
git-staged-changes = 已暫存的變更
git-unstaged-changes = 未暫存的變更
git-repository-clean = 工作區乾淨
git-select-file = 選取檔案以檢視差異
git-stage = 暫存
git-discard = 捨棄
git-recent-commits = 最近提交
git-no-commits = 尚無提交
git-local-branches = 本機分支
git-local = 本機
git-remote = 遠端
git-tags = 標籤
git-no-branches = 沒有本機分支
...
git-stage-all = 全部暫存
git-switch-workspace = 切換工作區
git-edit-config = 編輯設定檔
git-switch-recent-repository = 切換到最近的儲存庫
git-keybindings = 按鍵綁定
git-checkout = 簽出
git-new-branch = 新增分支
...
git-no-stashes = 沒有暫存
git-stash = 暫存變更
git-stash-pop = 套用並刪除暫存
git-stash-drop = 刪除暫存
git-amend = 修正提交
git-rebase = 重定基底
git-merge = 合併
git-fast-forward = 快轉
git-revert-commit = 還原提交
git-cherry-pick = 揀選提交
git-checkout-commit = 簽出提交
git-shortcut-panels = 面板
git-shortcut-navigate = 導覽
git-shortcut-help = 快速鍵
git-shortcut-title = Git 快速鍵
git-shortcut-description = 此 Git 頁面可用的快速鍵。
git-shortcut-universal = 通用
git-shortcut-next-panel = 下一個面板
git-shortcut-previous = 上一個項目
git-shortcut-next = 下一個項目
