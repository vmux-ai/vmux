...
mobile-demo-open = 沒有 Mac 也能看導覽
mobile-demo-exit = 結束導覽
mobile-status-demo = 導覽
mobile-demo-session-title = 修正訂單總額
mobile-demo-prompt = 多件商品的購物車中，訂單總額會差一分錢。你能找出原因嗎？
mobile-demo-thinking = 若每一列在加總前先四捨五入，就能解釋這一分錢的誤差。正在查看總額是如何計算的。
mobile-demo-answer = 加總被推斷成 float，因此每一列在相加前就先四捨五入了。把總額的型別指定為 Cents，即可維持整數運算，誤差也就消失了。
