...
mobile-demo-open = Tour ohne Mac starten
mobile-demo-exit = Tour verlassen
mobile-status-demo = Tour
mobile-demo-session-title = Bestellsummen korrigieren
mobile-demo-prompt = Bei Warenkörben mit mehreren Artikeln weicht die Bestellsumme um einen Cent ab. Findest du die Ursache?
mobile-demo-thinking = Würde jede Position vor dem Addieren gerundet, erklärte das eine Abweichung von einem Cent. Ich sehe mir an, wie die Summe gebildet wird.
mobile-demo-answer = Die Summe wurde als Fließkommazahl abgeleitet, daher wurde jede Position vor dem Addieren gerundet. Wird die Summe als Cents typisiert, bleibt sie in Ganzzahlarithmetik und die Abweichung verschwindet.
