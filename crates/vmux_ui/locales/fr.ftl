...
schema-update-channel = Canal de publication
schema-update-channel-detail = Stable reçoit les versions publiées. Aperçu reçoit les builds nocturnes.
schema-update-channel-stable = Stable
schema-update-channel-preview = Aperçu
...
mobile-nav-untitled-chat = Discussion sans titre
mobile-nav-launcher = Démarrer
mobile-nav-new-tab = Nouvel onglet
mobile-nav-unsupported = Indisponible sur cet appareil
mobile-nav-read-only = Lecture seule
mobile-nav-open-on-mac = Ouvrez-le sur votre Mac
mobile-nav-url-placeholder = Posez une question ou collez une URL…
...
git-commits = Commits
git-log = Journal
git-command-log = Journal des commandes
git-command-log-empty = Les opérations Git apparaîtront ici.
git-command-error = erreur
git-repository = Dépôt
git-changes = Modifications
git-branches = Branches Git
git-staged-changes = Modifications indexées
git-unstaged-changes = Modifications non indexées
git-repository-clean = L’arbre de travail est propre
git-select-file = Sélectionnez un fichier pour afficher ses différences
git-stage = Indexer
git-discard = Abandonner
git-recent-commits = Commits récents
git-no-commits = Aucun commit pour le moment
git-local-branches = Branches locales
git-local = Local
git-remote = Distant
git-tags = Étiquettes
git-no-branches = Aucune branche locale
...
git-switch-workspace = Changer d’espace de travail
git-edit-config = Modifier le fichier de configuration
git-switch-recent-repository = Basculer vers un dépôt récent
git-keybindings = Raccourcis clavier
git-checkout = Extraire
git-new-branch = Nouvelle branche
git-delete-branch = Supprimer la branche
git-stashes = Remisages
git-no-stashes = Aucun remisage
git-stash = Remiser les modifications
git-stash-pop = Appliquer et supprimer
git-stash-drop = Supprimer le remisage
git-amend = Modifier le commit
git-rebase = Rebaser
git-merge = Fusionner
git-fast-forward = Avance rapide
git-revert-commit = Annuler le commit
git-cherry-pick = Picorer
git-checkout-commit = Extraire le commit
git-shortcut-panels = Panneaux
git-shortcut-navigate = Naviguer
git-shortcut-help = Raccourcis
git-shortcut-title = Raccourcis Git
git-shortcut-description = Raccourcis disponibles pour cette page Git.
git-shortcut-universal = Général
git-shortcut-next-panel = Panneau suivant
git-shortcut-previous = Élément précédent
git-shortcut-next = Élément suivant
