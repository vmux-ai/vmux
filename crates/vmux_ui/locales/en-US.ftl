...
mobile-demo-open = Take a tour without a Mac
mobile-demo-exit = Exit tour
mobile-status-demo = Tour
mobile-demo-session-title = Fix order totals
mobile-demo-prompt = The order total is off by a cent on multi-item carts. Can you find it?
mobile-demo-thinking = Rounding each line before summing would explain a one-cent drift. Checking how the total is built.
mobile-demo-answer = The sum was inferring a float, so each line rounded before it was added. Typing the total as Cents keeps it in integer arithmetic and the drift goes away.
