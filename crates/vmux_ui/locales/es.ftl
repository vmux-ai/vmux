...
mobile-demo-open = Haz un recorrido sin Mac
mobile-demo-exit = Salir del recorrido
mobile-status-demo = Recorrido
mobile-demo-session-title = Corregir los totales de pedido
mobile-demo-prompt = El total del pedido se desvía un céntimo en carritos con varios artículos. ¿Puedes encontrar por qué?
mobile-demo-thinking = Redondear cada línea antes de sumarlas explicaría una desviación de un céntimo. Estoy viendo cómo se calcula el total.
mobile-demo-answer = La suma se infería como coma flotante, así que cada línea se redondeaba antes de sumarse. Si se tipa el total como Cents, el cálculo se mantiene en aritmética entera y la desviación desaparece.
