...
tools-title = Alati
tools-search = Pretraži pakete, agente, MCP, jezične alate i konfiguracijske datoteke…
...
tools-homebrew-sync = Instalirane formule i aplikacije automatski se sinkroniziraju.
...
tools-provider-language-tools = Jezični alati
tools-provider-mcp-servers = MCP poslužitelji
...
vault-description = Sinkronizirajte postavke, alate, dotfileove i znanje s Gitom.
vault-sync = Sinkronizacija
vault-create = Stvoriti
vault-connect = Poveži se
...
vault-empty = prazan
vault-clean = Ažurno
vault-not-connected = Nije povezano
...
schema-system = Sustav
schema-editor = Uređivač
schema-recording = Snimanje
schema-radius = Polumjer
schema-padding = Odmak
schema-gap = Razmak
schema-width = Širina
schema-color = Boja
schema-red = Crvena
...
schema-tidy-files = Sredi datoteke
...
layout-fold-knowledge = Sažmi znanje
layout-unfold-knowledge = Proširi znanje
...
layout-stack-number = Stog { $number }
layout-fold-stack = Sažmi stog
layout-unfold-stack = Proširi stog
layout-close-stack = Zatvori stog
...
error-page-failed-load = Učitavanje stranice nije uspjelo
...
command-interactive-mode-player = Scena > Interaktivni način > Igrač
command-minimize-window = Raspored > Prozor > Minimiziraj
command-toggle-layout = Raspored > Raspored > Uključi/isključi raspored
...
command-stack-close = Raspored > Stog > Zatvori stog
command-stack-next = Raspored > Stog > Sljedeći stog
command-stack-previous = Raspored > Stog > Prethodni stog
command-stack-reopen = Raspored > Stog > Ponovno otvori zatvorenu stranicu
command-stack-swap-prev = Raspored > Stog > Pomakni stog ulijevo
command-stack-swap-next = Raspored > Stog > Pomakni stog udesno
...
command-browser-prev-page = Preglednik > Navigacija > Natrag
command-browser-next-page = Preglednik > Navigacija > Naprijed
command-browser-reload = Preglednik > Navigacija > Ponovno učitaj
command-browser-hard-reload = Preglednik > Navigacija > Potpuno ponovno učitaj
command-open-in-place = Preglednik > Otvori > Otvori ovdje
command-open-in-new-stack = Preglednik > Otvori > Otvori u novom stogu
...
command-browser-dev-tools = Preglednik > Prikaz > Alati za razvojne programere
command-browser-open-command-bar = Preglednik > Traka > Traka naredbi
...
command-browser-open-commands = Preglednik > Traka > Naredbe
command-browser-open-history = Preglednik > Traka > Povijest
...
layout-folder-name = Naziv mape
layout-no-pins-bookmarks = Nema prikvačenih stranica ni oznaka
layout-move-to = Premjesti u { $folder }
layout-bookmark-current-page = Označi trenutačnu stranicu
layout-rename-folder = Preimenuj mapu
layout-remove-folder = Ukloni mapu
...
layout-restart-update = Ponovno pokreni za ažuriranje
...
agent-allow-tool = Dopustiti { $tool }?
agent-deny = Odbij
agent-allow-always = Uvijek dopusti
agent-allow = Dopusti
agent-loading-sessions = Učitavanje sesija…
agent-no-resumable-sessions = Nema sesija koje se mogu nastaviti
agent-no-matching-sessions = Nema podudarnih sesija
agent-no-matching-models = Nema podudarnih modela
agent-choice-help = ↑/↓ ili Ctrl+N/Ctrl+P · 1–9 · Enter
agent-choose-repository = Odaberite mapu repozitorija
agent-choose-repository-detail = Odaberite lokalni Git repozitorij koji agent treba koristiti.
agent-choosing = Odabir…
agent-choose-folder = Odaberite mapu
agent-queued = u redu čekanja
agent-attached = Priloženo:
agent-cancel-queued = Otkaži upit iz reda čekanja
agent-resume-queued = Nastavi upite iz reda čekanja
agent-clear-queue = Očisti red čekanja
...
agent-tool-searched-files = Pretraživao datoteke
agent-tool-ran-commands = Pokrenuo naredbe
...
agent-call = Poziv
agent-raw-event = Neobrađen događaj
agent-plan = Plan
agent-tasks = { $count ->
    [one] 1 zadatak
...
agent-status-running = Izvodi se
...
agent-slash-resume-session = Nastavi prošlu sesiju
...
agent-working-conjuring = Priziva
agent-working-cooking = Kuha
agent-working-brewing = Vari
agent-working-musing = Razmatra
agent-working-ruminating = Prebire
agent-working-scheming = Smišlja
agent-working-synthesizing = Sintetizira
agent-working-tinkering = Petlja
agent-working-churning = Vrti
agent-working-vibing = Hvata vibru
agent-working-simmering = Lagano kuha
agent-working-crafting = Izrađuje
agent-working-divining = Odgonetava
agent-working-mulling = Važe
agent-working-spelunking = Kopa po dubinama

editor-toggle-explorer = Uključi/isključi Explorer (Cmd+B)
editor-unsaved = nije spremljeno
editor-rendered-markdown = Prikazani Markdown s uređivanjem uživo
...
editor-open-externally = Otvori izvana
editor-changed-line = Promijenjeni redak
...
editor-delete-confirm = Izbrisati “{ $name }”? To se ne može poništiti.
editor-created-folder = Stvorena mapa { $name }
editor-created-file = Stvorena datoteka { $name }
editor-renamed-to = Preimenovano u { $name }
editor-deleted = Izbrisano { $name }
editor-failed-decode-image = Dekodiranje slike nije uspjelo
...
git-accept = ✓ prihvati
git-deny = ✗ odbij
git-show-unchanged-lines = Prikaži { $count } nepromijenjenih redaka

terminal-loading = Učitavanje…
terminal-runs-when-ready = pokreće se kad je spremno · Ctrl+C čisti · Esc preskače
terminal-booting = pokretanje
terminal-type-command = upišite naredbu · pokreće se kad je spremno · Esc preskače
...
setup-homebrew-required = Homebrew je potreban za instalaciju { $command }, a još nije postavljen. Vmux će prvo instalirati Homebrew, a zatim { $name }.
setup-terminal-instructions = U terminalu pritisnite Return za početak, a zatim unesite lozinku za Mac kad se zatraži.
setup-command-missing = Vmux je otvorio ovu stranicu jer lokalna naredba { $command } još nije instalirana. Pokrenite naredbu u nastavku da biste je instalirali.
setup-install-failed = Instalacija nije dovršena. Provjerite pojedinosti u terminalu, zatim pokušajte ponovno.
setup-installing = Instaliranje…
setup-install-homebrew = Instaliraj Homebrew + { $name }
setup-run-install = Pokreni naredbu za instalaciju
setup-auto-reload = Vmux je pokreće u terminalu i ponovno učitava kad je { $command } spreman.

debug-title = Otklanjanje pogrešaka
...
command-manage-spaces = Upravljanje prostorima…
command-pane-stack-location = okno { $pane } / sloj { $stack }
command-space-pane-stack-location = { $space } / okno { $pane } / sloj { $stack }
...
agents-terminal-coding-agent = Agent za kodiranje u terminalu
