...
knowledge-search = Bilimi gözläň
knowledge-no-match = Gabat gelýän bellikler ýok
knowledge-open-linked-note = Baglanan belligi açyň
knowledge-create-linked-note = Baglanan belligi dörediň
editor-properties = Sypatlar
editor-no-properties = Hiç hili häsiýet ýok
editor-add-tags = Bellikler goşuň
editor-add-property = Gozgalmaýan emläk goşuň
editor-change-property-type = Gozgalmaýan emläk görnüşini üýtgediň
editor-add-tag = Bellik goşuň
editor-add-item = Haryt goşuň
editor-linked-note = Baglanyşyk belligi
editor-property-value = Gymmatlyk
editor-delete-property = Gozgalmaýan emläk
editor-property-kind-text = Tekst
editor-property-kind-number = San
editor-property-kind-checkbox = Bellik gutusy
editor-property-kind-date = Sene
...
mobile-demo-open = Mac bolmazdan gezelenç ediň
mobile-demo-exit = Gezelençden çykyň
mobile-status-demo = Gezelenç
mobile-demo-session-title = Sargyt jemlerini düzediň
mobile-demo-prompt = Birnäçe haryt bar sebetlerde sargydyň jemi bir sent ýalňyş çykýar. Sebäbini tapyp bilersiňizmi?
mobile-demo-thinking = Jemlemezden öň her setiri tegeleklemek bir sentlik tapawudy düşündirip biler. Jemiň nähili hasaplanýandygyna seredýärin.
mobile-demo-answer = Jem float hökmünde kabul edilýärdi, şonuň üçin her setir goşulmazdan öň tegeleklenýärdi. Jeme Cents görnüşi berilse, ol bitin san arifmetikasynda galýar we tapawut ýitýär.
