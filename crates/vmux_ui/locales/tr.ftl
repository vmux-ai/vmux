...
knowledge-search = Bilgi Ara
knowledge-no-match = Eşleşen not yok
knowledge-open-linked-note = Bağlantılı notu aç
knowledge-create-linked-note = Bağlantılı not oluştur
editor-properties = Özellikler
editor-no-properties = Mülk yok
editor-add-tags = Etiket ekle
editor-add-property = Mülk ekle
editor-change-property-type = Mülk türünü değiştir
editor-add-tag = Etiket ekle
editor-add-item = Öğe ekle
editor-linked-note = Bağlantılı not
editor-property-value = Değer
editor-delete-property = Mülkü sil
editor-property-kind-text = Metin
editor-property-kind-number = Sayı
editor-property-kind-checkbox = Onay kutusu
editor-property-kind-date = Tarih
...
mobile-demo-open = Mac olmadan tura çık
mobile-demo-exit = Turdan çık
mobile-status-demo = Tur
mobile-demo-session-title = Sipariş toplamlarını düzelt
mobile-demo-prompt = Birden fazla ürün olan sepetlerde sipariş toplamı bir sent şaşıyor. Nedenini bulabilir misin?
mobile-demo-thinking = Toplamadan önce her satırın yuvarlanması bir sentlik sapmayı açıklar. Toplamın nasıl hesaplandığına bakıyorum.
mobile-demo-answer = Toplam float olarak çıkarsanıyordu, bu yüzden her satır eklenmeden önce yuvarlanıyordu. Toplama Cents türü verilince hesap tam sayı aritmetiğinde kalıyor ve sapma ortadan kalkıyor.
