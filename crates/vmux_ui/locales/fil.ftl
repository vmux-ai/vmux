...
mobile-demo-session-title = Ayusin ang mga total ng order
mobile-demo-prompt = Mali nang isang sentimo ang total ng order sa mga cart na maraming item. Makikita mo ba kung bakit?
mobile-demo-thinking = Kung niro-round ang bawat linya bago pagsamahin, maipapaliwanag nito ang isang sentimong pagkakaiba. Tinitingnan ko kung paano binubuo ang total.
mobile-demo-answer = Na-infer ang sum bilang float, kaya na-round ang bawat linya bago idagdag. Kapag tinipo ang total bilang Cents, mananatili itong nasa integer arithmetic at mawawala ang pagkakaiba.
