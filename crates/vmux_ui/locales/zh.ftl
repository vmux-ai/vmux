...
mobile-pair-title = 连接你的 Mac
mobile-pair-subtitle = 在 Vmux 里打开 Remote，选择 Connect device。
mobile-pair-scan = 扫描 QR 码
mobile-pair-show-link = 手动填写配对链接
mobile-pair-hide-link = 收起配对链接
mobile-pair-link-placeholder = 粘贴配对链接
mobile-pair-connect = 连接
mobile-pair-connecting = 连接中…
mobile-pair-disconnect = 断开
mobile-status-connected = 已连接
mobile-status-reaching = 正在联系你的 Mac…
mobile-status-disconnected = 尚未连接

mobile-url-invalid = 配对 URL 无效。
mobile-url-no-address = 配对 URL 缺少服务器地址。
mobile-url-bad-address = 配对 URL 的服务器地址无效。
mobile-url-no-token = 配对 URL 缺少令牌。
mobile-url-scheme = 配对 URL 必须使用 HTTPS 或 HTTP。
mobile-url-paste-full = 请粘贴 Vmux 在你的 Mac 上显示的完整配对 URL。

mobile-error-pairing-outdated = 该配对已过时。请重新扫描 Mac 上的 QR 码。
mobile-error-pairing-expired = 配对已失效。请重新扫描 Mac 上的 QR 码。
mobile-error-pairing-lost = 配对已失效。请重新与 Mac 配对。
mobile-error-token-rejected = 配对令牌已被拒绝。
mobile-error-stack-missing = 桌面端打开了聊天，但对应的堆栈没有出现。
mobile-error-not-offered = 你的 Mac 还不支持这项操作。
mobile-error-remote-disabled = 你的 Mac 已关闭 Remote。
mobile-error-session-gone = 该会话已不存在。
mobile-error-no-desktop = 请在 Mac 上打开 Vmux 窗口。
mobile-error-refused = 你的 Mac 未能完成该操作。
mobile-error-no-answer = 等待 { $seconds } 秒后仍未收到 { $address } 的回应。

mobile-start-stacks = 堆栈
mobile-start-no-stacks = 暂无打开的堆栈
mobile-start-team = 团队
mobile-start-search-placeholder = 搜索或提问…
mobile-start-new-chat = 开始聊天
mobile-start-starting = 启动中…

mobile-chat-back = 后退
mobile-chat-back-to-stacks = 返回堆栈
mobile-chat-no-messages = 还没有消息。
mobile-chat-no-session = 没有进行中的会话
mobile-chat-placeholder = 给智能体发消息…
mobile-chat-send = 发送
mobile-chat-stop = 停止
...
mobile-qr-camera-unavailable = 摄像头不可用。请改用配对链接。
...
mobile-qr-unsupported-platform = 该平台暂不支持 QR 扫描。请改用配对链接。
mobile-error-unexpected-answer = 你的 Mac 返回了意外响应。
mobile-error-connection-dropped = 与你的 Mac 的连接断开了。
...
mobile-demo-open = 无需 Mac 即可开始导览
mobile-demo-exit = 结束导览
mobile-status-demo = 导览
mobile-demo-session-title = 修正订单总额
mobile-demo-prompt = 购物车里商品一多，订单总额就差一分钱。你能找到原因吗？
mobile-demo-thinking = 每一行在相加之前先做了舍入，正好能解释一分钱的偏差。我来看看总额是怎么算的。
mobile-demo-answer = 总和被推断为 float，于是每一行在累加前就被舍入了。给总额标上 Cents 类型后，运算保持在整数上，偏差就消失了。
