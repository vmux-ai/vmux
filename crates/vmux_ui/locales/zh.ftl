...
layout-bookmark-in = 在 { $folder } 中添加书签
...
services-uptime-hours = { $hours } 小时 { $minutes } 分
services-uptime-days = { $days } 天 { $hours } 小时

error-page-failed-load = 页面加载失败
error-page-not-found = 页面未找到
error-unknown-host = 未知 Vmux 应用主机：{ $host }
...
command-equalize-pane-size = 布局 > 窗格 > 均分窗格大小
...
command-open-in-pane-top = 浏览器 > 打开 > 在上方窗格打开
command-open-in-pane-right = 浏览器 > 打开 > 在右侧窗格打开
command-open-in-pane-bottom = 浏览器 > 打开 > 在下方窗格打开
command-open-in-pane-left = 浏览器 > 打开 > 在左侧窗格打开
command-open-in-new-tab = 浏览器 > 打开 > 在新标签页打开
command-open-in-new-space = 浏览器 > 打开 > 在新空间打开
...
command-bookmark-pin-active = 书签 > 固定页面
...
layout-no-pins-bookmarks = 没有固定项或书签
...
agent-continued-from = 续接自 { $source }
...
agent-queued = 已排队
agent-attached = 已附加：
agent-cancel-queued = 取消排队的提示
agent-resume-queued = 恢复排队的提示
agent-clear-queue = 清空队列
agent-send-all-now = 立即全部发送
agent-choose-option = 选择上方选项
...
agent-worked-for-steps = { $count ->
    [one] 已工作 { $duration } · 1 步
   *[other] 已工作 { $duration } · { $count } 步
}
agent-tool-guardian-review = 守护审查
agent-tool-read-files = 读取文件
agent-tool-viewed-image = 查看图片
agent-tool-used-browser = 使用浏览器
agent-tool-searched-files = 搜索文件
agent-tool-ran-commands = 运行命令
agent-thinking = 正在思考
agent-subagent = 子智能体
agent-prompt = 提示
agent-thread = 线程
...
agent-working-working = 正在处理
agent-working-thinking = 正在思考
agent-working-pondering = 正在琢磨
agent-working-noodling = 正在构思
agent-working-percolating = 正在酝酿
agent-working-conjuring = 正在生成
agent-working-cooking = 正在加工
agent-working-brewing = 正在酝酿
agent-working-musing = 正在沉思
agent-working-ruminating = 正在推敲
agent-working-scheming = 正在规划
agent-working-synthesizing = 正在综合
agent-working-tinkering = 正在调试
agent-working-churning = 正在运转
agent-working-vibing = 正在找感觉
agent-working-simmering = 正在细化
agent-working-crafting = 正在打磨
agent-working-divining = 正在探查
agent-working-mulling = 正在斟酌
...
editor-open-externally = 用外部应用打开
...
editor-preview-large-image = 图片（过大，无法预览）
editor-preview-binary = 二进制文件
...
terminal-runs-when-ready = 准备好后运行 · Ctrl+C 清除 · Esc 跳过
terminal-booting = 正在启动
terminal-type-command = 输入命令 · 准备好后运行 · Esc 跳过

setup-tagline-claude = Anthropic 的编码智能体，就在 Vmux
setup-tagline-codex = OpenAI 的编码智能体，就在 Vmux
setup-tagline-vibe = Mistral 的编码智能体，就在 Vmux
setup-install-title = 安装 { $name } CLI
setup-homebrew-required = 安装 { $command } 需要 Homebrew，但尚未设置。Vmux 将先安装 Homebrew，再安装 { $name }。
setup-terminal-instructions = 在终端中按 Return 开始，然后按提示输入你的 Mac 密码。
setup-command-missing = Vmux 打开此页面，是因为本地尚未安装 { $command } 命令。运行下方命令即可获取。
...
agents-terminal-coding-agent = 基于终端的编码 Agent
