...
mobile-demo-open = Пройти обзор без Mac
mobile-demo-exit = Выйти из обзора
mobile-status-demo = Обзор
mobile-demo-session-title = Исправить итоги заказов
mobile-demo-prompt = Итог заказа расходится на цент в корзинах с несколькими товарами. Найдёте причину?
mobile-demo-thinking = Округление каждой строки до суммирования объяснило бы расхождение в один цент. Смотрю, как считается итог.
mobile-demo-answer = Сумма выводилась как float, поэтому каждая строка округлялась до сложения. Если задать итогу тип Cents, вычисления остаются целочисленными и расхождение исчезает.
