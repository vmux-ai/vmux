...
vault-private = 私有倉庫
vault-public-warning = 儲存庫歷程與加密資料將會公開。
...
mobile-demo-open = 不用 Mac 也能參觀導覽
mobile-demo-exit = 離開導覽
mobile-status-demo = 導覽
mobile-demo-session-title = 修正訂單金額加總
mobile-demo-prompt = 購物車有多項商品時，訂單金額會少算一分錢。你找得出原因嗎？
mobile-demo-thinking = 每一項在加總之前先四捨五入，就會造成一分錢的落差。我來看看金額是怎麼算出來的。
mobile-demo-answer = 總和被推斷為 float，所以每一項在加進去之前就先被四捨五入。將總額標註成 Cents 型別後，運算會維持在整數，落差也就不見了。
