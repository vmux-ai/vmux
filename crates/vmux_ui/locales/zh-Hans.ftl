...
mobile-demo-open = 没有 Mac 也能看导览
mobile-demo-exit = 退出导览
mobile-status-demo = 导览
mobile-demo-session-title = 修复订单金额合计
mobile-demo-prompt = 多件商品的购物车里，订单合计会差一分钱。你能找出原因吗？
mobile-demo-thinking = 如果每行在求和前先四舍五入，就能解释这一分钱的偏差。正在查看合计是怎么算出来的。
mobile-demo-answer = 求和被推断成了 float，所以每行在相加前就被舍入了。把合计的类型定为 Cents，就能保持整数运算，偏差也随之消失。
