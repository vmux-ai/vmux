...
schema-update-channel = 发布频道
schema-update-channel-detail = 稳定版接收正式发布，预览版接收每日构建。
schema-update-channel-stable = 稳定版
schema-update-channel-preview = 预览版
...
command-rotate-forward = 布局 > 窗格 > 向前轮换窗格
command-rotate-backward = 布局 > 窗格 > 向后轮换窗格
command-mirror-panes = 布局 > 窗格 > 镜像窗格
command-mirror-panes-horizontal = 布局 > 窗格 > 水平镜像窗格
command-mirror-panes-vertical = 布局 > 窗格 > 垂直镜像窗格
...
mobile-nav-untitled-chat = 未命名对话
mobile-nav-launcher = 开始
mobile-nav-new-tab = 新标签页
mobile-nav-unsupported = 此设备不支持
mobile-nav-read-only = 只读
mobile-nav-open-on-mac = 请在 Mac 上打开
mobile-nav-url-placeholder = 提问或粘贴网址…
...
git-commits = 提交
git-log = 日志
git-command-log = 命令日志
git-command-log-empty = Git 操作将显示在此处。
git-command-error = 错误
git-repository = 仓库
git-changes = 更改
git-branches = 分支
git-staged-changes = 已暂存的更改
git-unstaged-changes = 未暂存的更改
git-repository-clean = 工作区干净
git-select-file = 选择文件以查看差异
git-stage = 暂存
git-discard = 丢弃
git-recent-commits = 最近提交
git-no-commits = 暂无提交
git-local-branches = 本地分支
git-local = 本地
git-remote = 远程
git-tags = 标签
git-no-branches = 暂无本地分支
...
git-stage-all = 全部暂存
git-switch-workspace = 切换工作区
git-edit-config = 编辑配置文件
git-switch-recent-repository = 切换到最近的仓库
git-keybindings = 按键绑定
git-checkout = 检出
git-new-branch = 新建分支
...
git-shortcut-panels = 面板
git-shortcut-navigate = 导航
git-shortcut-help = 快捷键
git-shortcut-title = Git 快捷键
git-shortcut-description = 此 Git 页面可用的快捷键。
git-shortcut-universal = 通用
git-shortcut-next-panel = 下一个面板
git-shortcut-previous = 上一项
git-shortcut-next = 下一项
