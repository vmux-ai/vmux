...
schema-update-channel = Canal de lançamento
schema-update-channel-detail = Estável recebe versões publicadas. Prévia recebe builds noturnas.
schema-update-channel-stable = Estável
schema-update-channel-preview = Prévia
...
mobile-nav-untitled-chat = Conversa sem título
mobile-nav-launcher = Início
mobile-nav-new-tab = Nova aba
mobile-nav-unsupported = Indisponível neste dispositivo
mobile-nav-read-only = Somente leitura
mobile-nav-open-on-mac = Abra no seu Mac
mobile-nav-url-placeholder = Pergunte ou cole uma URL…
...
git-commits = Commits
git-log = Histórico
git-command-log = Log de comandos
git-command-log-empty = As operações do Git aparecerão aqui.
git-command-error = erro
git-repository = Repositório
git-changes = Alterações
git-branches = Ramificações
git-staged-changes = Alterações preparadas
git-unstaged-changes = Alterações não preparadas
git-repository-clean = Árvore de trabalho limpa
git-select-file = Selecione um arquivo para ver o diff
git-stage = Preparar
git-discard = Descartar
git-recent-commits = Commits recentes
git-no-commits = Nenhum commit ainda
git-local-branches = Ramificações locais
git-local = Local
git-remote = Remoto
git-tags = Tags
git-no-branches = Nenhuma ramificação local
...
git-stage-all = Preparar tudo
git-switch-workspace = Trocar espaço de trabalho
git-edit-config = Editar arquivo de configuração
git-switch-recent-repository = Trocar para um repositório recente
git-keybindings = Atalhos de teclado
git-checkout = Fazer checkout
git-new-branch = Nova branch
git-delete-branch = Excluir branch
git-stashes = Stashes
git-no-stashes = Nenhum stash
git-stash = Guardar alterações
git-stash-pop = Aplicar e remover stash
git-stash-drop = Excluir stash
git-amend = Corrigir commit
git-rebase = Rebase
git-merge = Mesclar
git-fast-forward = Avanço rápido
git-revert-commit = Reverter commit
git-cherry-pick = Cherry-pick
git-checkout-commit = Fazer checkout do commit
git-shortcut-panels = Painéis
git-shortcut-navigate = Navegar
git-shortcut-help = Atalhos
git-shortcut-title = Atalhos do Git
git-shortcut-description = Atalhos disponíveis para esta página Git.
git-shortcut-universal = Geral
git-shortcut-next-panel = Próximo painel
git-shortcut-previous = Item anterior
git-shortcut-next = Próximo item
