...
schema-update-channel = 릴리스 채널
schema-update-channel-detail = 안정 채널은 정식 릴리스를 받고, 미리 보기 채널은 나이틀리 빌드를 받습니다.
schema-update-channel-stable = 안정
schema-update-channel-preview = 미리 보기
...
command-rotate-forward = Layout > Pane > Pane 앞으로 회전
command-rotate-backward = Layout > Pane > Pane 뒤로 회전
command-mirror-panes = Layout > Pane > Pane 반전
command-mirror-panes-horizontal = Layout > Pane > Pane 가로 반전
command-mirror-panes-vertical = Layout > Pane > Pane 세로 반전
...
git-changes = 변경 사항
git-branches = 브랜치
git-staged-changes = 스테이징된 변경 사항
git-unstaged-changes = 스테이징되지 않은 변경 사항
git-repository-clean = 작업 트리가 깨끗합니다
git-select-file = diff를 확인할 파일을 선택하세요
git-stage = 스테이징
git-discard = 변경 취소
git-recent-commits = 최근 커밋
git-no-commits = 아직 커밋이 없습니다
git-local-branches = 로컬 브랜치
git-local = 로컬
git-remote = 원격
git-tags = 태그
git-no-branches = 로컬 브랜치가 없습니다
...
git-stage-all = 모두 스테이지
git-switch-workspace = 작업 공간 전환
git-edit-config = 설정 파일 편집
git-switch-recent-repository = 최근 저장소로 전환
git-keybindings = 키 바인딩
git-checkout = 체크아웃
git-new-branch = 새 브랜치
git-delete-branch = 브랜치 삭제
git-stashes = 스태시
git-no-stashes = 스태시 없음
git-stash = 변경 사항 스태시
git-stash-pop = 스태시 적용 및 삭제
git-stash-drop = 스태시 삭제
git-amend = 커밋 수정
git-rebase = 리베이스
git-merge = 병합
git-fast-forward = 빨리 감기
git-revert-commit = 커밋 되돌리기
git-cherry-pick = 체리픽
git-checkout-commit = 커밋 체크아웃
git-shortcut-panels = 패널
git-shortcut-navigate = 이동
git-shortcut-help = 단축키
git-shortcut-title = Git 단축키
git-shortcut-description = 이 Git 페이지에서 사용할 수 있는 단축키입니다.
git-shortcut-universal = 공통
git-shortcut-next-panel = 다음 패널
git-shortcut-previous = 이전 항목
git-shortcut-next = 다음 항목
