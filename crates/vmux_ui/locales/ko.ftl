...
mobile-demo-open = Mac 없이 둘러보기
mobile-demo-exit = 둘러보기 종료
mobile-status-demo = 둘러보기
mobile-demo-session-title = 주문 합계 오류 수정
mobile-demo-prompt = 여러 상품이 담긴 장바구니에서 주문 합계가 1센트 어긋납니다. 원인을 찾아줄 수 있나요?
mobile-demo-thinking = 각 항목을 더하기 전에 반올림하면 1센트 오차가 생길 수 있습니다. 합계가 어떻게 계산되는지 확인하고 있습니다.
mobile-demo-answer = 합계가 부동소수점으로 추론되어 각 항목이 더해지기 전에 반올림되었습니다. 합계를 Cents 타입으로 지정하면 정수 연산이 유지되어 오차가 사라집니다.
