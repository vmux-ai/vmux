...
schema-update-channel = Kanał wydań
schema-update-channel-detail = Kanał stabilny otrzymuje opublikowane wydania, a podgląd — kompilacje nocne.
schema-update-channel-stable = Stabilny
schema-update-channel-preview = Podgląd
...
command-rotate-forward = Układ > Panel > Obróć panele do przodu
command-rotate-backward = Układ > Panel > Obróć panele do tyłu
command-mirror-panes = Układ > Panel > Odbij panele
command-mirror-panes-horizontal = Układ > Panel > Odbij panele poziomo
command-mirror-panes-vertical = Układ > Panel > Odbij panele pionowo
...
mobile-nav-untitled-chat = Czat bez tytułu
mobile-nav-launcher = Start
mobile-nav-new-tab = Nowa karta
mobile-nav-unsupported = Niedostępne na tym urządzeniu
mobile-nav-read-only = Tylko do odczytu
mobile-nav-open-on-mac = Otwórz na Macu
mobile-nav-url-placeholder = Zapytaj lub wklej adres URL…
...
git-commits = Commity
git-log = Dziennik
git-command-log = Dziennik poleceń
git-command-log-empty = Operacje Git pojawią się tutaj.
git-command-error = błąd
git-repository = Repozytorium
git-changes = Zmiany
git-branches = Gałęzie
git-staged-changes = Zmiany w indeksie
git-unstaged-changes = Zmiany poza indeksem
git-repository-clean = Drzewo robocze jest czyste
git-select-file = Wybierz plik, aby wyświetlić różnice
git-stage = Dodaj do indeksu
git-discard = Odrzuć
git-recent-commits = Ostatnie commity
git-no-commits = Brak commitów
git-local-branches = Lokalne gałęzie
git-local = Lokalne
git-remote = Zdalne
git-tags = Tagi
git-no-branches = Brak lokalnych gałęzi
...
git-stage-all = Dodaj wszystko do stagingu
git-switch-workspace = Przełącz obszar roboczy
git-edit-config = Edytuj plik konfiguracji
git-switch-recent-repository = Przełącz na ostatnie repozytorium
git-keybindings = Skróty klawiszowe
git-checkout = Przełącz
git-new-branch = Nowa gałąź
git-delete-branch = Usuń gałąź
git-stashes = Schowki
git-no-stashes = Brak schowków
git-stash = Schowaj zmiany
git-stash-pop = Zastosuj i usuń schowek
git-stash-drop = Usuń schowek
git-amend = Popraw commit
git-rebase = Rebase
git-merge = Scal
git-fast-forward = Fast-forward
git-revert-commit = Odwróć commit
git-cherry-pick = Cherry-pick
git-checkout-commit = Przełącz na commit
git-shortcut-panels = Panele
git-shortcut-navigate = Nawigacja
git-shortcut-help = Skróty
git-shortcut-title = Skróty Git
git-shortcut-description = Dostępne skróty dla tej strony Git.
git-shortcut-universal = Ogólne
git-shortcut-next-panel = Następny panel
git-shortcut-previous = Poprzedni element
git-shortcut-next = Następny element
