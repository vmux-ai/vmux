...
use vmux_core::tool::{
    ...
    ToolInstallRequest, ToolItem, ToolLinkRequest, ToolOpenRequest, ToolOperationKind,
    ToolProvider, ToolStatus, ToolUninstallRequest, ToolUnlinkRequest, ToolUpdateRequest,
    ToolsNavigateRequest, ToolsSnapshot,
};
use vmux_tool::{
    ExternalToolOperation, ToolOperationFailed, ToolOperationFinished, ToolOperationRequest,
    ToolOperationSucceeded, ToolScanOutput, ToolScanRequest, ToolStore, ToolStoreTarget,
    ToolsManifest,
};

pub(crate) struct DesktopToolPlugin;

impl Plugin for DesktopToolPlugin {
    fn build(&self, app: &mut App) {
        app.add_observer(on_navigate_request)
            .add_observer(on_open_request)
            .add_systems(
                Update,
                (start_tool_scan, finish_tool_scan, finish_tool_operation).chain(),
            )
            .add_systems(
                Update,
                (
...
#[derive(Component)]
struct DesktopToolScanTask(Task<DesktopToolScanOutput>);

struct DesktopToolScanOutput {
    snapshot: ToolsSnapshot,
    manifest: ToolsManifest,
}

#[derive(Component)]
struct DesktopToolOperationTask(Task<Result<String, String>>);
...
fn start_external_tool_operation<R: ExternalToolRequest>(
    ...
    for (entity, operation, target) in &operations {
        ...
        commands
            ...
            .insert(DesktopToolOperationTask(task));
    }
}

fn start_tool_scan(
    scans: Query<(Entity, &ToolScanRequest, &ToolStoreTarget), Added<ToolScanRequest>>,
    stores: Query<&ToolStore>,
    mut commands: Commands,
) {
    for (entity, request, target) in &scans {
        let Ok(store) = stores.get(target.0).cloned() else {
            continue;
        };
        let refresh_catalogs = request.refresh_catalogs();
        let task = IoTaskPool::get().spawn(async move { scan_tools(&store, refresh_catalogs) });
        commands.entity(entity).insert(DesktopToolScanTask(task));
    }
}

fn finish_tool_scan(mut scans: Query<(Entity, &mut DesktopToolScanTask)>, mut commands: Commands) {
    for (entity, mut task) in &mut scans {
        let Some(output) = future::block_on(future::poll_once(&mut task.0)) else {
            continue;
        };
        commands
            .entity(entity)
            .remove::<DesktopToolScanTask>()
            .insert(ToolScanOutput::new(output.snapshot, output.manifest));
    }
}

fn finish_tool_operation(
    mut operations: Query<(Entity, &mut DesktopToolOperationTask)>,
    mut commands: Commands,
) {
    for (entity, mut task) in &mut operations {
        let Some(result) = future::block_on(future::poll_once(&mut task.0)) else {
            continue;
        };
        let mut operation = commands.entity(entity);
        operation.remove::<DesktopToolOperationTask>();
        match result {
            Ok(message) => {
                operation.insert((ToolOperationFinished, ToolOperationSucceeded(message)));
            }
            Err(message) => {
                operation.insert((ToolOperationFinished, ToolOperationFailed(message)));
            }
        }
    }
}

fn scan_tools(store: &ToolStore, refresh_catalogs: bool) -> DesktopToolScanOutput {
