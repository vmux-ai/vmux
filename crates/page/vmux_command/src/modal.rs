//! The `vmux://command-bar/` modal page.
//!
//! [`Page`] is the Cmd+K modal served at `vmux://command-bar/`, and everything here exists to
//! make a webview of its own behave like a modal: measuring the shell so the host can size the
//! native window around it, the reveal handshake, and dismissing on an outside pointer.
//!
//! `web`, not `ui`. The palette this renders is portable and lives in [`crate::page`]; the modal
//! around it is not, and does not need to be — the surface a user actually opens is
//! [`crate::panel::CommandBarPanel`], drawn inside the host page rather than in a webview.
