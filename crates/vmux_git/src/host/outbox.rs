...
impl GitOutbox {
    ...
    pub(super) fn events(&self, webview: Entity, emits: Vec<Emit>) {
        self.push(GitOutboxItem { webview, emits });
    }
...
fn drain_git_outbox(
    ...
    for GitOutboxItem { webview, emits } in outbox.drain() {
        for emit in emits {
...
mod tests {
    ...
    fn drain_empties_outbox() {
        let mut app = App::new();
        app.add_plugins(MinimalPlugins).add_plugins(OutboxPlugin);
