...
use crate::ui_state::{GitCommandLogEntry, GitPageSnapshot, GitUiStateEvent};

pub(super) struct GitViewPlugin;

impl Plugin for GitViewPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(vmux_core::host::UiStatePlugin::<GitUiStateEvent>::default())
            .add_observer(on_page_ready)
            .add_systems(Update, publish_git_ui_state);
    }
}

type GitUiStateUpdates = vmux_core::host::UiStateUpdates<GitUiStateEvent>;
...
fn on_page_ready(
    ...
    commands.entity(entity).insert(GitView::default());
}

fn publish_git_ui_state(
    views: Query<(Entity, &GitView), Changed<GitView>>,
    mut commands: Commands,
) {
