...
///
/// The relay's own control port is discarded — that one is for desktops registering, and a phone
/// sending there reaches no particular Mac.
pub fn relay_base_url(relay_url: &str, allocated_port: u16) -> Result<String, String> {
    let parsed = url::Url::parse(relay_url).map_err(|error| error.to_string())?;
    let host = parsed.host_str().ok_or("relay url has no host")?;
    let scheme = parsed.scheme();
    Ok(format!("{scheme}://{host}:{allocated_port}"))
}
...
pub fn pairing_info(base_url: &str, token: &str, fingerprint: &str) -> Result<PairingInfo, String> {
    ...
    let mut deep_link = url::Url::parse("vmuxremote://pair").map_err(|error| error.to_string())?;
    deep_link
        .query_pairs_mut()
...
pub fn allocated_port() -> Option<u16> {
...
pub fn certificate_fingerprint() -> Option<String> {
...
///
/// launchd starts the daemon, so it inherits no environment; whoever enables Remote has to write
/// down what `VMUX_REMOTE_RELAY_URL` said or the daemon will only ever see the default.
pub fn persist_relay_url(relay_url: &str) -> std::io::Result<()> {
...
mod tests {
    ...
    fn a_base_url_takes_the_relay_host_and_the_allocated_port() {
        assert_eq!(
            relay_base_url("https://relay.vmux.ai", 41003).unwrap(),
            "https://relay.vmux.ai:41003"
        );
        // The relay's own control port is replaced, not appended to.
        assert_eq!(
            relay_base_url("https://localhost:8787", 41003).unwrap(),
    ...
    fn a_fingerprint_reaches_both_pairing_shapes() {
        let pairing = pairing_info("https://localhost:41003", "secret", "abc123").unwrap();
    ...
    fn an_absent_fingerprint_leaves_both_shapes_well_formed() {
        let pairing = pairing_info("https://localhost:41003", "secret", "").unwrap();
