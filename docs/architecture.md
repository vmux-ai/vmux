...
plugin; the plugin attaches private targeted observer systems that own typed argument validation
and command or query dispatch. Publication and execution query those entities directly; there is
no separate runtime registry or central function-pointer table.
