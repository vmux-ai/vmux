...
7. **Side-sheet space card** — shows the space `startup_dir` when the tab has no worktree; when it does, a **read-only worktree chip** (`TabBoundary` via `push_tab_boundary_emit`; `is_worktree`/`branch` git-detected via `is_linked_worktree`, cached by dir; `TabWorktree` supplies `base_ref`). Merged into the space card, header pinned `shrink-0`. **No UI buttons** — worktree lifecycle is agent-driven (create via the MCP tool; removal by asking the agent, self-healing via git detection). `BoundaryCommandEvent`/`on_boundary_command_emit` removed. (The async `CreateTabWorktreeRequest`/`RemoveTabWorktreeRequest` orchestration in `vmux_layout::worktree` is retained but currently unused — kept for a future `remove_worktree` MCP tool; create uses the sync GUI handler.)
