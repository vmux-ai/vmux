...
## Status (all phases attempted per request)

**Shipped + test-verified (native `cargo test` + wasm `cargo check -p vmux_server --target wasm32`):**

- **Phase 1 — Identity (§1).** `AvatarSpec::for_registry`/`Profile::registry`/`hash_color`; `team::Agent.kind → Option`; ACP attach inserts `Profile`+`Agent`+registry favicon (`PageMetadata.icon`); `acp_icon_for_id` sources it from `AcpCatalog`. → ACP appears in the team roster and tab strip with its registry avatar.
- **Phase 2 (DOM half) — Chat header avatar (§2.4).** `ChatSnapshot` carries `agent_name`/`agent_icon`/`accent_color`; chat header + empty-state hero render the favicon avatar + real name (fixes Page agents too).
- **Phase 3 — Notifications (§4, C1/C6).** `consume_page_agent_stream` raises `AgentAttention` on Streaming→Idle → done-dot + OS notify (ACP + Page); `surface_errors` + `attach_last_run_state_kind` now cover `AcpSession` (ACP error toast). C5 (correct "{name} finished") fell out of Phase 1 since `mark_agent_done` reads `Profile`.
- **Phase 4 — Command bar + polish (§5).** `vmux_core::AgentWorkingDir` feeds ACP cwd into the command-bar current-work; chat status dot reflects `AgentRunState`; loading dots take the brand accent.

**Deferred (documented — not safely completable/verifiable in an autonomous pass):**

- **Phase 2b — Native macOS focus-ring badge with the registry icon (§2.2 / gap A2, B1).** Needs a heavy SVG-raster dep (`usvg`/`resvg`), async icon fetch + on-disk cache, and an invasive `ActiveStack.kind → identity` change rippling through `windowed_ring_for`/`sync_windowed_frames` and every `ActivatePane` emitter — rendered via the vendored `patches/bevy_cef_core` CALayer, so it can't be runtime-verified here. Current behaviour degrades gracefully (ACP follow panes get the colored ring, just no logo). The registry icon is already shown on every DOM surface. **Follow-up PR.**
- **C4 — Notify-tool caller resolution for ACP (§4).** ACP tool calls carry an `anchor` `ProcessId` (on the child webview), but `AgentSpaceWriters.agents` resolves callers by a session-entity `ProcessId`/`sid`. A safe fix needs `AcpSession`-aware resolution (adding the anchor to the stack would double-match the follow resolvers). Minor: the auto Streaming→Idle path (C1) already covers "finished". **Follow-up.**
- **Phase 5 — Session resume (§6).** Persisting the sid/transcript needs `Reflect` on `AcpSession` (whose `cwd: PathBuf` / `anchor: ProcessId` aren't `Reflect`) and `AgentMessages`; actual backend continuity needs ACP `loadSession` support in the daemon (`vmux_service/acp`) which isn't present. Without the daemon side this delivers only display-continuity — half a feature. **Own spec/PR.**
